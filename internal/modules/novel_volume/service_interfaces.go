...
type VolumeService interface {
	...
	CreateVolume(ctx context.Context, novelID uuid.UUID, title string, description, coverImageURL *string, displayOrder int, isPublished bool, createdBy uuid.UUID) (*domain.Volume, error)

	// UpdateVolume updates volume information with history tracking
	UpdateVolume(ctx context.Context, id uuid.UUID, volumeNumber int, title string, description, coverImageURL *string, displayOrder int, isPublished bool, changedBy uuid.UUID, requestContext map[string]any) (*domain.Volume, error)
	...
	GetVolumeByID(ctx context.Context, id uuid.UUID) (*domain.Volume, error)

	// GetVolumesByNovelID retrieves all volumes for a novel
	GetVolumesByNovelID(ctx context.Context, novelID uuid.UUID, publishedOnly bool) ([]*domain.Volume, error)
