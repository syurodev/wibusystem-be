...
func (h *Handler) PasskeyRegisterBegin(c *gin.Context) {
	...
	if !exists {
		response.Error(c, http.StatusUnauthorized, "UNAUTHORIZED", I18nUnauthorized, nil)
		return
	}

	userID, err := uuid.FromString(userIDStr.(string))
	if err != nil {
		response.Error(c, http.StatusBadRequest, "INVALID_USER_ID", I18nInvalidUserID, nil)
		return
	}
	...
	if err != nil {
		...
		response.Error(c, http.StatusInternalServerError, "REGISTRATION_BEGIN_FAILED", I18nRegistrationBeginFailed, nil)
		return
	}
...
func (h *Handler) PasskeyRegisterFinish(c *gin.Context) {
	...
	if err != nil {
		...
		response.Error(c, http.StatusBadRequest, "INVALID_CREDENTIAL_DATA", I18nInvalidCredentialData, nil)
		return
	}
	...
	if err != nil {
		if errors.Is(err, pkgerrors.ErrInvalidOrExpiredSession) {
			response.Error(c, http.StatusBadRequest, "INVALID_SESSION", I18nInvalidOrExpiredSession, nil)
			return
		}
		if errors.Is(err, pkgerrors.ErrCredentialAlreadyExists) {
			response.Error(c, http.StatusConflict, "CREDENTIAL_EXISTS", I18nCredentialAlreadyExists, nil)
			return
		}
		if errors.Is(err, pkgerrors.ErrCredentialVerificationFailed) {
			response.Error(c, http.StatusBadRequest, "VERIFICATION_FAILED", I18nCredentialVerificationFailed, nil)
			return
		}
		zap.L().Error("Failed to finish passkey registration", zap.Error(err), zap.String("user_id", userID.String()))
		response.Error(c, http.StatusInternalServerError, "REGISTRATION_FAILED", I18nRegistrationFailed, nil)
		return
	}
	...
	response.Success(c, http.StatusCreated, I18nPasskeyRegistered, resp, nil)
}
...
func (h *Handler) PasskeyAuthenticateBegin(c *gin.Context) {
	...
	if err != nil {
		...
		response.Error(c, http.StatusUnauthorized, "AUTHENTICATION_FAILED", I18nAuthenticationFailed, nil)
		return
	}
	...
	if err != nil {
		if errors.Is(err, pkgerrors.ErrNoPasskeyRegistered) {
			response.Error(c, http.StatusNotFound, "NO_PASSKEY", I18nNoPasskeyRegistered, nil)
			return
		}
		zap.L().Error("Failed to begin passkey authentication", zap.Error(err), zap.String("user_id", user.ID.String()))
		response.Error(c, http.StatusInternalServerError, "AUTHENTICATION_BEGIN_FAILED", I18nAuthenticationBeginFailed, nil)
		return
	}
...
func (h *Handler) PasskeyAuthenticateFinish(c *gin.Context) {
	...
	if userIDStr == "" {
		response.Error(c, http.StatusBadRequest, "MISSING_USER_ID", I18nMissingUserID, nil)
		return
	}
	...
	if err != nil {
		...
		response.Error(c, http.StatusInternalServerError, "AUTHENTICATION_FAILED", I18nAuthenticationFailed, nil)
		return
	}
	...
	if err != nil {
		...
		response.Error(c, http.StatusInternalServerError, "SESSION_CREATION_FAILED", I18nSessionCreationFailed, nil)
		return
	}
	...
	response.Success(c, http.StatusOK, I18nAuthenticationSuccess, resp, nil)
}
...
func (h *Handler) PasskeyListCredentials(c *gin.Context) {
	...
	if err != nil {
		...
		response.Error(c, http.StatusInternalServerError, "LIST_FAILED", I18nListCredentialsFailed, nil)
		return
	}
	...
	response.Success(c, http.StatusOK, I18nCredentialsListed, resp, nil)
}
...
func (h *Handler) PasskeyDeleteCredential(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, "INVALID_CREDENTIAL_ID", I18nInvalidCredentialID, nil)
		return
	}
	...
	if err := h.webauthnService.DeleteCredential(c.Request.Context(), userID, credentialID); err != nil {
		if errors.Is(err, pkgerrors.ErrCredentialNotFound) {
			response.Error(c, http.StatusNotFound, "CREDENTIAL_NOT_FOUND", I18nCredentialNotFound, nil)
			return
		}
		zap.L().Error("Failed to delete credential", zap.Error(err), zap.String("user_id", userID.String()))
		response.Error(c, http.StatusInternalServerError, "DELETE_FAILED", I18nDeleteCredentialFailed, nil)
		return
	}

	response.Success(c, http.StatusOK, I18nCredentialDeleted, nil, nil)
}
...
func (h *Handler) PasskeyUpdateCredentialName(c *gin.Context) {
	...
	if err := h.webauthnService.UpdateCredentialName(c.Request.Context(), userID, credentialID, req.CredentialName); err != nil {
		...
		response.Error(c, http.StatusInternalServerError, "UPDATE_FAILED", I18nUpdateCredentialFailed, nil)
		return
	}

	response.Success(c, http.StatusOK, I18nCredentialUpdated, nil, nil)
}
