...
// LocaleMiddleware creates a Fiber middleware that automatically resolves language preferences
// and sets up the localizer for each request.
func LocaleMiddleware(translator *Translator) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if currentCookie := c.Cookies(config.CookieName); currentCookie != resolvedLanguage {
...
func RequireLanguage(translator *Translator, requiredLanguage string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		config := translator.GetConfig()
