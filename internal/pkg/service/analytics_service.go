...
func (s *AnalyticsService) GetTopTrending(ctx context.Context, mediaType string, timeRange string, limit int) ([]map[string]any, error) {
	...
	if len(trendingData) == 0 {
		// Fallback: If no trending data, return top viewed items
	...
	// 3. Fetch details from Postgres
	novelMap := make(map[uuid.UUID]*domain.Novel)
	if len(novelIDs) > 0 {
		novels, _, err := s.novelRepo.List(ctx, domain.NovelFilter{IDs: novelIDs, Limit: len(novelIDs)})
		if err != nil {
...
func (s *AnalyticsService) mapNovelToMediaSeries(n *domain.Novel, views uint64) map[string]any {
