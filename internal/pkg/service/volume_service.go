...
func NewVolumeService(volumeRepo domain.VolumeRepository, historyRepo VolumeHistoryRepository) *VolumeService {
...
func (s *VolumeService) DeleteVolume(ctx context.Context, id uuid.UUID) error {
	// Check if volume exists
	_, err := s.volumeRepo.GetByID(ctx, id)
	if err != nil {
		if err == pgx.ErrNoRows {
			return pkgerrors.ErrVolumeNotFound
		}
		return err
	}
...
func (s *VolumeService) GetVolumeByID(ctx context.Context, id uuid.UUID) (*domain.Volume, error) {
...
func (s *VolumeService) GetVolumesByNovelID(ctx context.Context, novelID uuid.UUID, publishedOnly bool) ([]*domain.Volume, error) {
	return s.volumeRepo.GetByNovelID(ctx, novelID, publishedOnly)
}

// UpdateDisplayOrder updates the display order of a volume
func (s *VolumeService) UpdateDisplayOrder(ctx context.Context, id uuid.UUID, order int) error {
	// Check if volume exists
	_, err := s.volumeRepo.GetByID(ctx, id)
	if err != nil {
		if err == pgx.ErrNoRows {
