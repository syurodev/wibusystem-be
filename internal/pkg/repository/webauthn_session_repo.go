...
func (r *webauthnSessionRepository) GetByChallenge(ctx context.Context, challenge string) (*domain.WebAuthnSession, error) {
	...
	rows, err := r.pool.Query(ctx, query, challenge, time.Now())
	if err != nil {
		return nil, err
	}

	session, err := pgx.CollectOneRow(rows, pgx.RowToAddrOfStructByName[domain.WebAuthnSession])
	if err != nil {
		return nil, err
	}
...
func (r *webauthnSessionRepository) Delete(ctx context.Context, id uuid.UUID) error {
	query := `DELETE FROM identify.webauthn_sessions WHERE id = $1`
...
func (r *webauthnSessionRepository) DeleteExpired(ctx context.Context) error {
	query := `DELETE FROM identify.webauthn_sessions WHERE expires_at <= $1`
