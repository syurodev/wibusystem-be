...
func (a *AttestationObject) VerifyAttestation(clientDataHash []byte, mds metadata.Provider) (err error) {
	...
	if AttestationFormat(a.Format) == AttestationFormatNone {
		// DEBUG: Log to verify patch is active
		fmt.Printf("DEBUG: Attestation format is 'none', attStmt len=%d\n", len(a.AttStatement))
		if len(a.AttStatement) != 0 {
                // PATCHED: Skip strict check for none attestation - browser compatibility
		}
